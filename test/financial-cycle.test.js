...
test("bootstrap hydrates saved settings and converts the exclusive end to the visible inclusive date", async () => {
	const controller = createWizardController({
		adapter: { load: async () => ({ selectedPeriod: period, incomeAmount: 900000 }) },
	});
	...
	assert.deepEqual(
		...
		{ startDate: "2028-02-29", endDate: "2028-02-29", incomeAmount: 900000, configured: true },
	);
});
...
test("one save persists period and blank income without invoking completion or synchronization", async () => {
	...
	const controller = createWizardController({
		adapter: {
			load: async () => null,
			save: async (...args) => { calls.push(args); return { selectedPeriod: args[0], incomeAmount: args[1] }; },
			complete: async () => { throw new Error("must not complete or sync"); },
		},
	});
	await controller.save({ startDate: "2028-02-29", endDate: "2028-02-29", incomeAmount: "" });
	assert.deepEqual(calls, [[period, null]]);
	assert.deepEqual(
		{ period: controller.state.period, incomeAmount: controller.state.incomeAmount, configured: controller.state.configured, outcome: controller.state.outcome },
...
test("income must be a positive whole CLP amount when provided", () => {
	for (const value of ["12.5", "0", "-1", "pesos"]) {
		let state = wizardReducer(undefined, { type: "SET_FORM", startDate: "2028-02-29", endDate: "2028-02-29", incomeAmount: value });
		state = wizardReducer(state, { type: "VALIDATE" });
		assert.equal(state.errorField, "incomeAmount");
	}
	let state = wizardReducer(undefined, { type: "SET_FORM", startDate: "2028-02-29", endDate: "2028-02-29", incomeAmount: "900000" });
	state = wizardReducer(state, { type: "VALIDATE" });
...
test("invalid periods identify the date field", () => {
	let state = wizardReducer(undefined, { type: "SET_FORM", startDate: "2028-03-02", endDate: "2028-03-01", incomeAmount: "" });
	state = wizardReducer(state, { type: "VALIDATE" });
...
test("duplicate save submissions share one in-flight settings request", async () => {
	...
	const controller = createWizardController({
		adapter: { save: async () => { saves += 1; await pending; return {}; } },
	});
	const input = { startDate: "2028-02-29", endDate: "2028-02-29", incomeAmount: "" };
...
test("save failures leave first setup blocking and retryable", async () => {
	const controller = createWizardController({ adapter: { save: async () => ({ outcome: "error" }) } });
	await controller.save({ startDate: "2028-02-29", endDate: "2028-02-29", incomeAmount: "" });
...
test("authenticated adapter only uses the financial-cycle settings endpoint while demo performs no write", async () => {
	...
	const authenticated = createAuthenticatedAdapter(async (url, options = {}) => {
		...
		return { ok: true, json: async () => ({ selectedPeriod: period, incomeAmount: null }) };
	});
	await authenticated.load();
	await authenticated.save(period, null);
	assert.deepEqual(calls, [["/api/financial-cycle", "GET"], ["/api/financial-cycle", "PUT"]]);
	assert.deepEqual(await createDemoAdapter().save(period, null), { selectedPeriod: period, incomeAmount: null });
});

test("focus helpers focus heading, trap tab, and restore the invoker", () => {
	let focused = "";
	const heading = { focus: () => { focused = "heading"; } };
	const invoker = { focus: () => { focused = "invoker"; } };
